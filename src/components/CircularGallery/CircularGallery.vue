...
class Media {
  ...
  constructor({
    ...
    this.grayscaleTarget = grayscale ? 1.0 : 0.0;
    this.createShader();
    this.createMesh();
    this.createTitle();
    this.onResize();
  ...
  update(
    ...
    if (this.grayscale) {
      this.program.uniforms.uGrayscale.value = lerp(
        this.program.uniforms.uGrayscale.value,
        this.grayscaleTarget,
        0.08,
      );
    }
...
class App {
  ...
  onWheel(e: Event) {
    ...
    const legacy = wheelEvent as unknown as {
      wheelDelta?: number;
      detail?: number;
    };
  ...
  update() {
    ...
    if (this.grayscale && this.mouseInside && this.medias) {
      const worldX =
        (this.mouse.x / this.screen.width - 0.5) * this.viewport.width;
      const worldY =
        -(this.mouse.y / this.screen.height - 0.5) * this.viewport.height;
      ...
      this.medias.forEach((media) => {
        ...
        media.grayscaleTarget = isHovered ? 0.0 : 1.0;
      });
    }
