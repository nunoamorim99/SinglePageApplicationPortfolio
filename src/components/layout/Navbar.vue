...
<template>
  <header
    ...
    :class="scrolled
      ...
      : 'bg-transparent'"
  >
    <nav class="section-container flex h-20 items-center justify-between" :aria-label="t('nav.home')">
      <!-- Logo -->
      <router-link
        to="/"
      ...
      <div class="flex items-center gap-1">
        ...
        <div class="lang-switcher relative ml-2">
          ...
          >
            <img :src="currentFlag" :alt="locale" class="h-3.5 w-5 rounded-sm object-cover" />
            {{ locale }}
          </button>
          <Transition
          ...
          >
            <ul
              v-if="langOpen"
              class="absolute right-0 top-full mt-2 min-w-[120px] overflow-hidden border border-charcoal-100 bg-cream-50 py-1 dark:border-charcoal-700 dark:bg-charcoal-800"
              role="listbox"
            >
              <li v-for="loc in locales" :key="loc.code" role="option" :aria-selected="locale === loc.code">
                ...
                >
                  <img :src="flagUrl(loc.flag)" :alt="loc.code" class="h-3.5 w-5 rounded-sm object-cover" />
                  {{ t(`lang.${loc.code}`) }}
                </button>
              </li>
            </ul>
        ...
        <ul class="flex flex-col gap-1">
          <li v-for="link in navLinks" :key="link.key">
            <router-link
              ...
              :class="isActive(link.to) ? 'text-vermillion' : 'text-charcoal-500 dark:text-charcoal-300'"
              @click="ui.closeMobileMenu()"
            >
              {{ t(`nav.${link.key}`) }}
            </router-link>
