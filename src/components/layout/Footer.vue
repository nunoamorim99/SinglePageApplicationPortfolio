...
<template>
  <footer class="border-t border-charcoal-100 bg-charcoal text-cream-200 dark:border-charcoal-700 dark:bg-charcoal-900">
    <div class="section-container py-16">
      <div class="flex flex-col gap-12 lg:flex-row lg:items-end lg:justify-between">
        <div>
          <p class="font-serif text-display-sm text-cream-100">
            {{ profile.name }}<span class="text-vermillion italic">.</span>
          </p>
          <p class="mt-2 text-sm tracking-wide text-charcoal-300">{{ t('hero.role') }} — {{ profile.location }}</p>
        </div>
        <div class="flex items-center gap-6">
          <a :href="profile.linkedin" target="_blank" rel="noopener noreferrer" aria-label="LinkedIn"
            class="text-charcoal-400 transition-colors duration-300 hover:text-vermillion">
      ...
      <div class="mt-12 border-t border-charcoal-700 pt-8 flex flex-col items-center justify-between gap-4 sm:flex-row">
        <p class="text-xs tracking-widest uppercase text-charcoal-500">
