  ...
  <div class="pt-24 pb-20">
    <div class="section-container">
      ...
      <div class="text-center">
        ...
        <p class="section-subtitle">Staying active, setting goals, pushing limits</p>
      </div>
      ...
      <div class="mt-16 grid gap-8 sm:grid-cols-2">
        <div v-for="sport in sports" :key="sport.id" class="card">
          <div class="flex items-center gap-3">
            ...
            <h3 class="font-display text-xl font-semibold text-gray-900">{{ sport.name }}</h3>
          </div>
          <p class="mt-3 text-sm leading-relaxed text-gray-600">{{ sport.description }}</p>
          <div class="mt-4">
      ...
      <!-- Achievements -->
      <div class="mt-20">
        <h2 class="text-center font-display text-2xl font-bold text-gray-900">Achievements & Milestones</h2>
        <div class="mx-auto mt-10 max-w-2xl">
          <div class="relative border-l-2 border-primary-100 pl-8">
            <div
              v-for="(achievement, index) in achievements"
              :key="index"
              class="relative mb-10 last:mb-0"
            >
              <div class="absolute -left-[calc(2rem+5px)] top-1 h-3 w-3 rounded-full border-2 border-primary-500 bg-white" />
              <span class="text-xs font-semibold uppercase tracking-wider text-primary-600">{{ achievement.year }}</span>
              <h3 class="mt-1 font-display text-lg font-semibold text-gray-900">{{ achievement.title }}</h3>
